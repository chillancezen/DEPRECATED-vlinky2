...
uint64_t map_shared_memory_from_sockets(char * name,int length, unsigned long *nodemask)
{
	...
	int fd,rc=0;
	char filename[256];
	length=POWER_ROUND(length);/*length must be power of 2*/
	length=PAGE_ALIGN(4096,length);
	...
	if(base!=MAP_FAILED){
		rc=mbind(base+length, length,MPOL_BIND,nodemask,64,MPOL_MF_STRICT|MPOL_MF_MOVE_ALL);
	}
	printf("[x]map %s with length:%d memory bind mask:%"PRIx64" result:%d\n",filename,length,(uint64_t)*nodemask,rc);
	close(fd);
